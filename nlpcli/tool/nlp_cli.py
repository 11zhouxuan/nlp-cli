...
def parse_args():
    ...
    while True:
        ...
        if tokenizer_type in tokenizer_type_choices:
            ...
            break
        if tokenizer_type in [str(i) for i in range(len(tokenizer_type_choices))]:
            args['tokenizer_type'] = tokenizer_type_choices[int(tokenizer_type)]
            break
    ...
    while True:
        ...
        if model_construct_type in model_construct_choices:
            ...
            break
        if model_construct_type in [str(i) for i in range(len(model_construct_choices))]:
            args['model_construct_type'] = model_construct_choices[int(model_construct_type)]
            break
...
#     # 生成相关的文件夹
#     _generator_directory(path_project)
